...
sum(cp.cost_to_customer_per_qty*cp.quantity) as money_spent
...
with x as (
select customer_id, strftime('%m', market_date) as month, strftime('%Y', market_date) as year,
cost_to_customer_per_qty*quantity as money_spent
from customer_purchases)
select customer_id, sum(money_spent)
from x
